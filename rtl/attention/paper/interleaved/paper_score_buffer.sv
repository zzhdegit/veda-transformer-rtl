...
module paper_score_buffer #(
    ...
    input  h9_score_packet_t     in_packet,

    output logic                 out_valid,
    input  logic                 out_ready,
    output h9_score_packet_t     out_packet,
    ...
    h9_score_packet_t mem [0:DEPTH-1];
    ...
    h9_score_packet_t out_packet_q;
    logic out_valid_q;
    ...
    initial begin
        if (DEPTH <= 0 || COUNTER_W <= 0) begin
            $fatal(1, "paper_score_buffer parameters must be positive");
        end
    end
...
`ifndef SYNTHESIS
    always_ff @(posedge clk) begin
        if (rst_n && !clear) begin
            assert (!(in_fire && full))
                else $error("paper_score_buffer no_score_overflow failed");
            assert (!(out_fire && empty && !out_valid_q))
                else $error("paper_score_buffer no_score_underflow failed");
            if ($past(rst_n) && $past(in_valid && !in_ready)) begin
                assert (in_valid)
                    else $error("paper_score_buffer score_payload_stable_until_ready valid failed");
                assert ($stable(in_packet))
                    else $error("paper_score_buffer score_payload_stable_until_ready payload failed");
            end
            if ($past(rst_n) && $past(out_valid && !out_ready)) begin
                assert (out_valid)
                    else $error("paper_score_buffer output valid dropped under backpressure");
                assert ($stable(out_packet))
                    else $error("paper_score_buffer output payload changed under backpressure");
            end
            assert (!(out_valid && $isunknown(out_packet)))
                else $error("paper_score_buffer no_unknown_output_when_valid failed");
        end
    end
`endif
endmodule
