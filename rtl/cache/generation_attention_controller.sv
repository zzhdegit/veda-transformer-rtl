...
module generation_attention_controller #(
    ...
    localparam logic [7:0] STATUS_DIM_ERROR  = 8'h81;
    ...
    logic [15:0] q_stage [0:D_HEAD-1];
    logic [15:0] k_stage [0:D_HEAD-1];
    logic [15:0] v_stage [0:D_HEAD-1];
    ...
    initial begin
        if (PE_NUM <= 0 || D_HEAD <= 0 || MAX_SEQ_LEN <= 0 || META_W <= 0 || COUNTER_W <= 0) begin
            $fatal(1, "generation_attention_controller parameters must be positive");
        end
        if ((PE_NUM & (PE_NUM - 1)) != 0) begin
            $fatal(1, "generation_attention_controller PE_NUM must be a power of two");
        end
    end
    ...
    assign cache_provisional_read_enable =
        (state_q inside {ST_CACHE_READ_REQ, ST_CACHE_READ_RSP, ST_LOAD_K, ST_LOAD_V,
                         ST_START_ATTENTION, ST_ATTENTION_RUN});
    ...
    assign cache_append_k_fp16 = k_stage[int'(append_dim_q)];
    assign cache_append_v_fp16 = v_stage[int'(append_dim_q)];
    assign cache_append_last_dim = append_last_dim;
    assign cache_append_complete = append_last_dim;
    ...
    assign sha_load_data = (state_q == ST_LOAD_Q) ? q_stage[int'(load_dim_q)] :
    ...
    assign output_last = sha_output_last;
            ...
            unique case (state_q)
                ST_LOAD_TOKEN: begin
                    if (token_fire) begin
                        ...
                        if ((token_dim != input_dim_q) || (token_last_dim != input_last_dim_expected)) begin
                            done_valid_q <= 1'b1;
                            done_status_q <= STATUS_DIM_ERROR;
                            done_invalid_q <= 1'b1;
                            done_meta_q <= (input_dim_q == '0) ? token_meta : meta_q;
                            done_valid_seq_len_q <= cache_valid_seq_len;
                            input_dim_q <= '0;
                            state_q <= ST_DONE;
                        end else begin
                            q_stage[int'(token_dim)] <= token_q_fp16;
                            k_stage[int'(token_dim)] <= token_k_fp16;
                            v_stage[int'(token_dim)] <= token_v_fp16;

                            if (input_last_dim_expected) begin
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            perf_provisional_append_cycles <= '0;
            perf_attention_cycles <= '0;
            ...
            if (state_q == ST_PROVISIONAL_APPEND) begin
                perf_provisional_append_cycles <= perf_provisional_append_cycles + COUNTER_W'(1);
            end
            if (state_q inside {ST_LOAD_Q, ST_CACHE_READ_REQ, ST_CACHE_READ_RSP,
                                ST_LOAD_K, ST_LOAD_V, ST_START_ATTENTION,
                                ST_ATTENTION_RUN}) begin
                perf_attention_cycles <= perf_attention_cycles + COUNTER_W'(1);
            end
...
`ifndef SYNTHESIS
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        ...
        end else if (token_fire && input_last_dim_expected && (token_dim == input_dim_q) && (token_last_dim == input_last_dim_expected)) begin
    ...
    always_ff @(posedge clk) begin
        if (rst_n) begin
            assert (!(sha_start_valid && (sha_start_seq_len == '0)))
                else $error("generation_attention_controller no zero-length Stage 3 start failed");
            assert (!(sha_start_valid && !cache_provisional_valid))
                else $error("generation_attention_controller no_attention_without_complete_current_kv failed");
            assert (!(sha_start_valid && cache_append_incomplete))
                else $error("generation_attention_controller no_attention_start_during_incomplete_append failed");
            assert (!(sha_start_valid && (sha_start_seq_len != (cache_valid_seq_len + SEQ_LEN_W'(1)))))
                else $error("generation_attention_controller attention_seq_len_equals_valid_seq_len_plus_one failed");
            assert (!(cache_append_valid && (cache_append_token_index != cache_valid_seq_len[TOKEN_W-1:0])))
                else $error("generation_attention_controller provisional_token_index_equals_valid_seq_len failed");
            assert (!(cache_commit_valid && !attention_done_seen_q))
                else $error("generation_attention_controller no_commit_before_attention_done failed");
            assert (!((state_q inside {ST_PROVISIONAL_APPEND, ST_LOAD_Q, ST_CACHE_READ_REQ,
                ...
                else $error("generation_attention_controller no_valid_seq_len_increment_before_commit failed");
            assert (!(cache_rd_valid && (SEQ_LEN_W'(cache_rd_token) >= attention_seq_len)))
                else $error("generation_attention_controller no_read_beyond_valid_seq_len failed");
            assert (!(cache_rd_valid && (SEQ_LEN_W'(cache_rd_token) == seq_len_snapshot_q) && !cache_provisional_valid))
                else $error("generation_attention_controller no_provisional_visibility_before_complete failed");
            assert (!(cache_rd_valid && (SEQ_LEN_W'(cache_rd_token) == seq_len_snapshot_q) && !cache_provisional_read_enable))
                else $error("generation_attention_controller current_token_read_allowed_only_for_active_operation failed");
            assert (!(cache_full && cache_append_valid))
                else $error("generation_attention_controller no_overwrite_when_full failed");
            assert (!(sha_start_valid && (seq_len_snapshot_q == '0) && (sha_start_seq_len != SEQ_LEN_W'(1))))
                else $error("generation_attention_controller first_token_not_empty_attention failed");
            assert (!(sha_load_valid && (state_q == ST_LOAD_K) && (cache_rd_rsp_token != read_token_q)))
                else $error("generation_attention_controller K/V token alignment failed");
            assert (!(output_valid && $isunknown({output_base_dim, output_vector_fp32, output_lane_mask,
                ...
                else $error("generation_attention_controller no unknown output when valid failed");
            assert (!(done_valid && $isunknown({done_status, done_invalid, done_meta, done_valid_seq_len})))
                else $error("generation_attention_controller no unknown done when valid failed");
            assert (COUNTER_W'(accepted_token_count) >= perf_generation_steps)
                else $error("generation_attention_controller transaction count conserved failed");

            if ($past(rst_n) && $past(token_valid && !token_ready)) begin
                assert (token_valid)
                    else $error("generation_attention_controller token valid dropped under backpressure");
                assert ($stable({token_dim, token_q_fp16, token_k_fp16, token_v_fp16, token_last_dim, token_meta}))
                    else $error("generation_attention_controller token payload changed under backpressure");
            end
            if ($past(rst_n) && $past(output_valid && !output_ready)) begin
                assert (output_valid)
                    else $error("generation_attention_controller output valid dropped under backpressure");
                assert ($stable({output_base_dim, output_vector_fp32, output_lane_mask,
                                 output_status, output_invalid, output_meta, output_last}))
                    else $error("generation_attention_controller output stable under backpressure failed");
            end
            if ($past(rst_n) && $past(done_valid && !done_ready)) begin
                assert (done_valid)
                    else $error("generation_attention_controller done valid dropped under backpressure");
                assert ($stable({done_status, done_invalid, done_meta, done_valid_seq_len}))
                    else $error("generation_attention_controller done stable under backpressure failed");
            end
        end
    end
`endif
