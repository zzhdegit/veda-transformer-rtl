...
module fp32_sqrt_wrapper #(
    ...
    initial begin
        if (META_W <= 0) begin
            $fatal(1, "fp32_sqrt_wrapper META_W must be positive");
        end
        if (LATENCY != 1) begin
            $fatal(1, "fp32_sqrt_wrapper currently exposes exactly one output register stage");
        end
        if (INITIATION_INTERVAL != 1) begin
            $fatal(1, "fp32_sqrt_wrapper currently supports initiation interval 1");
        end
    end

    assign a_nonfinite = in_a[30:23] == 8'hFF;
    assign a_negative_nonzero = in_a[31] && (in_a[30:0] != 31'd0);
    assign invalid_comb = a_nonfinite || a_negative_nonzero;
    assign dw_a = invalid_comb ? FP32_ZERO : in_a;
...
`ifndef SYNTHESIS
    ...
    always_ff @(posedge clk) begin
        if (rst_n && ASSERT_ON_INVALID && input_fire) begin
            assert (!invalid_comb)
                else $fatal(1, "fp32_sqrt_wrapper unsupported NaN/Inf/negative input assertion failed");
        end
    end
`endif
endmodule
