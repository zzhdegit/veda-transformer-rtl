...
module fp32_add_wrapper #(
    ...
    initial begin
        if (META_W <= 0) begin
            $fatal(1, "fp32_add_wrapper META_W must be positive");
        end
        if (LATENCY != 1) begin
            $fatal(1, "fp32_add_wrapper currently exposes exactly one output register stage");
        end
    ...
    assign invalid_comb = a_nonfinite || b_nonfinite;
...
`ifndef SYNTHESIS
    ...
    always_ff @(posedge clk) begin
        if (rst_n && ASSERT_ON_INVALID && input_fire) begin
            assert (!invalid_comb || pipe_in_data[40])
                else $error("fp32_add_wrapper no_invalid_input_without_flag failed");
            assert (!invalid_comb)
                else $fatal(1, "fp32_add_wrapper unsupported NaN/Inf input assertion failed");
        end
    end
`endif
endmodule
