...
BUILD_DIR="$ROOT_DIR/build/stage7d_rtl_sim"
REPORT_DIR="$ROOT_DIR/reports/stage_07"
SUMMARY="$REPORT_DIR/phase_7d_vcs_rtl_sim.txt"
...
{
  echo "Stage 7D full Pre-Norm transformer_layer RTL simulation"
  echo "Build dir: build/stage7d_rtl_sim"
...
compile_and_run_layer() {
  ...
  (cd "$BUILD_DIR" && vcs -full64 -sverilog -debug_access+pp -assert svaext -timescale=1ns/1ps \
    +define+STAGE7_N_HEAD="$n_head" +define+STAGE7_D_HEAD="$d_head" \
...
compile_and_run_layer layer_h2_d16 2 16 "$VECTOR_DIR/stage7d_h2_d16.mem" || failures=$((failures + 1))

{
  echo
  echo "Run markers:"
  for log in "$BUILD_DIR"/*_run.log; do
    grep -E "(STAGE7D_|CHECK_FAIL)" "$log" || true
  done
  echo "result=$([ "$failures" -eq 0 ] && echo PASS || echo FAIL)"
  echo "Full logs: build/stage7d_rtl_sim/*_compile.log and *_run.log"
} >> "$SUMMARY"
