...
REPORT = ROOT / "reports/stage_01b/test_results.txt"
...
def run_python_test_fallback():
    test_files = sorted((ROOT / "tb" / "model").glob("test_*.py")) + sorted(
        (ROOT / "tb" / "unit").glob("test_*.py")
    )
    failures = []
    executed = 0
    ...
    for index, path in enumerate(test_files):
        module_name = "_stage1b_fallback_%d_%s" % (index, path.stem)
...
def main():
    ...
    if shutil.which("pytest") or run([sys.executable, "-m", "pytest", "--version"])[0] == 0:
        ...
        code, output = run_python_test_fallback()
        lines.append("$ fallback-python-test-runner tb/model tb/unit")
        lines.append(output)
    ...
    if shutil.which("vcs"):
        cmd = ["bash", str(ROOT / "scripts" / "sim" / "run_stage1b_vcs.sh")]
        code, output = run(cmd)
        lines.append("$ " + " ".join(cmd))
        lines.append(output.rstrip())
        lines.append("exit_code=%d" % code)
        ...
        lines.append("Stage 1B RTL simulation: SKIPPED - vcs executable not found.")
