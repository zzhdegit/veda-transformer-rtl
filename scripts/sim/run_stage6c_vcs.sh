...
BUILD_DIR="$ROOT_DIR/build/stage6c_rtl_sim"
REPORT_DIR="$ROOT_DIR/reports/stage_06"
SUMMARY="$REPORT_DIR/phase_6c_vcs_rtl_sim.txt"
...
{
  echo "Stage 6C VCS RTL simulation"
  echo "Build dir: build/stage6c_rtl_sim"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/sim/gen_stage6c_vectors.py" "$VECTOR_DIR" > "$BUILD_DIR/vector_gen.log" 2>&1
vector_code=$?
...
compile_and_run_qkv() {
  ...
  (cd "$BUILD_DIR" && vcs -full64 -sverilog -debug_access+pp -assert svaext -timescale=1ns/1ps \
    ...
    -o "$simv" \
    "${DW_FILES[@]}" "${COMMON_RTL[@]}" "${ARITH_RTL[@]}" "${PE_RTL[@]}" "${PROJ_RTL[@]}" \
    "$ROOT_DIR/tb/rtl/stage6/tb_qkv_projection_engine.sv" \
    -top tb_qkv_projection_engine \
    -l "$compile_log")
  local compile_code=$?
  ...
  timeout 600s "$simv" "+QKV_VECTOR_FILE=$vector_file" -l "$run_log"
  local run_code=$?
  local log_errors=0
  if grep -E "(STAGE6C_.*FAIL|CHECK_FAIL|Fatal:|Error:|assert.*failed|unsupported .* assertion failed)" "$run_log" >/dev/null 2>&1; then
    log_errors=1
  fi
...
compile_and_run_qkv qkv_h1_d8 1 8 "$VECTOR_DIR/stage6c_qkv_h1_d8.mem" || failures=$((failures + 1))
compile_and_run_qkv qkv_h2_d8 2 8 "$VECTOR_DIR/stage6c_qkv_h2_d8.mem" || failures=$((failures + 1))
compile_and_run_qkv qkv_h4_d8 4 8 "$VECTOR_DIR/stage6c_qkv_h4_d8.mem" || failures=$((failures + 1))
compile_and_run_qkv qkv_h2_d16 2 16 "$VECTOR_DIR/stage6c_qkv_h2_d16.mem" || failures=$((failures + 1))

{
  echo
  echo "Run markers:"
  for log in "$BUILD_DIR"/*_run.log; do
    grep -E "(STAGE6C_|CHECK_FAIL)" "$log" || true
  done
  echo "result=$([ "$failures" -eq 0 ] && echo PASS || echo FAIL)"
  echo "Full logs: build/stage6c_rtl_sim/*_compile.log and *_run.log"
} >> "$SUMMARY"
