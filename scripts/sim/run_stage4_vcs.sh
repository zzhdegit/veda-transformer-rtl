...
BUILD_DIR="$ROOT_DIR/build/stage4_rtl_sim"
REPORT_DIR="$ROOT_DIR/reports/stage_04"
...
{
  echo "Stage 4 VCS RTL simulation"
  echo "Build dir: build/stage4_rtl_sim"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/sim/gen_stage4_vectors.py" "$VECTOR_DIR" > "$BUILD_DIR/vector_gen.log" 2>&1
vector_code=$?
...
compile_and_run_cache() {
  ...
  (cd "$BUILD_DIR" && vcs -full64 -sverilog -debug_access+pp -assert svaext -timescale=1ns/1ps \
    ...
    "$ROOT_DIR/rtl/cache/kv_address_generator.sv" \
    "$ROOT_DIR/rtl/cache/kv_cache_manager.sv" \
    "$ROOT_DIR/tb/rtl/stage4/tb_kv_cache_manager.sv" \
    -top tb_kv_cache_manager \
    -l "$compile_log")
  local compile_code=$?
  ...
  timeout 180s "$simv" -l "$run_log"
  local run_code=$?
  local log_errors=0
  if grep -E "(STAGE4_.*FAIL|CHECK_FAIL|Fatal:|assert.*failed|unsupported .* assertion failed)" "$run_log" >/dev/null 2>&1; then
    log_errors=1
  fi
...
compile_and_run_generation() {
  ...
  (cd "$BUILD_DIR" && vcs -full64 -sverilog -debug_access+pp -assert svaext -timescale=1ns/1ps \
    ...
    "${ATTN_RTL[@]}" \
    "${CACHE_RTL[@]}" \
    "$ROOT_DIR/tb/rtl/stage4/tb_generation_attention_engine.sv" \
    -top tb_generation_attention_engine \
    -l "$compile_log")
  local compile_code=$?
  if [ "$compile_code" -ne 0 ]; then
    echo "$name compile_exit_code=$compile_code" >> "$SUMMARY"
  ...
  timeout 720s "$simv" \
    +GENERATION_VECTOR_FILE="$VECTOR_DIR/$vector_file" \
    -l "$run_log"
  local run_code=$?
  local log_errors=0
  if grep -E "(STAGE4_.*FAIL|CHECK_FAIL|Fatal:|assert.*failed|unsupported .* assertion failed)" "$run_log" >/dev/null 2>&1; then
...
compile_and_run_cache kv_cache_manager || failures=$((failures + 1))
compile_and_run_generation generation_attention_d8 8 stage4_generation_d8.mem || failures=$((failures + 1))
compile_and_run_generation generation_attention_d16 16 stage4_generation_d16.mem || failures=$((failures + 1))

{
  echo
  echo "Run markers:"
  for log in "$BUILD_DIR"/*_run.log; do
    grep -E "(STAGE4_|CHECK_FAIL)" "$log" || true
  done
  echo "result=$([ "$failures" -eq 0 ] && echo PASS || echo FAIL)"
  echo "Full logs: build/stage4_rtl_sim/*_compile.log and *_run.log"
} >> "$SUMMARY"
