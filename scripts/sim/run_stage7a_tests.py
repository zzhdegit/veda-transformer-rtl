...
REPORT = ROOT / "reports/stage_07/phase_7a_test_results.txt"
...
def run_python_test_fallback(test_files):
    ...
    for index, path in enumerate(test_files):
        module_name = "_stage7a_fallback_%d_%s" % (index, path.stem)
...
def main():
    ...
    lines = ["Stage 7A Pre-Norm Transformer layer specification/model regression", ""]
    failures = 0
    test_files = [
        ROOT / "tb/model/test_stage7_transformer_reference.py",
        ROOT / "tb/model/test_stage6e_output_projection.py",
    ]
    ...
    if shutil.which("pytest") or run([sys.executable, "-m", "pytest", "--version"])[0] == 0:
        ...
        lines.append("$ fallback-python-test-runner stage7a/stage6e model tests")
        lines.append(output)
    ...
    commands.append([sys.executable, "model/transformer/transformer_layer_cycle_model.py"])
    ...
    lines.append("Stage 7A Python/model result: %s" % ("FAIL" if failures else "PASS"))
    REPORT.write_text("\n".join(lines) + "\n", encoding="utf-8")
