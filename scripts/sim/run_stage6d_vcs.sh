...
BUILD_DIR="$ROOT_DIR/build/stage6d_rtl_sim"
REPORT_DIR="$ROOT_DIR/reports/stage_06"
SUMMARY="$REPORT_DIR/phase_6d_vcs_rtl_sim.txt"
...
{
  echo "Stage 6D VCS RTL simulation"
  echo "Build dir: build/stage6d_rtl_sim"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/sim/gen_stage6d_vectors.py" "$VECTOR_DIR" > "$BUILD_DIR/vector_gen.log" 2>&1
vector_code=$?
...
PROJ_RTL=(
  ...
  "$ROOT_DIR/rtl/projection/qkv_projection_engine.sv"
  "$ROOT_DIR/rtl/attention/projected_multi_head_attention.sv"
)

compile_and_run_projected() {
  local name=$1
  local n_head=$2
  local d_head=$3
  local vector_file=$4
  ...
  (cd "$BUILD_DIR" && vcs -full64 -sverilog -debug_access+pp -assert svaext -timescale=1ns/1ps \
    ...
    "$ROOT_DIR/tb/rtl/stage6/tb_projected_multi_head_attention.sv" \
    -top tb_projected_multi_head_attention \
    -l "$compile_log")
  local compile_code=$?
  ...
  timeout 900s "$simv" "+PROJECTED_MHA_VECTOR_FILE=$vector_file" -l "$run_log"
  local run_code=$?
  local log_errors=0
  if grep -E "(STAGE6D_.*FAIL|CHECK_FAIL|Fatal:|Error:|assert.*failed|unsupported .* assertion failed)" "$run_log" >/dev/null 2>&1; then
    log_errors=1
  fi
...
compile_and_run_projected projected_h1_d8 1 8 "$VECTOR_DIR/stage6d_projected_mha_h1_d8.mem" || failures=$((failures + 1))
compile_and_run_projected projected_h2_d8 2 8 "$VECTOR_DIR/stage6d_projected_mha_h2_d8.mem" || failures=$((failures + 1))
compile_and_run_projected projected_h4_d8 4 8 "$VECTOR_DIR/stage6d_projected_mha_h4_d8.mem" || failures=$((failures + 1))
compile_and_run_projected projected_h2_d16 2 16 "$VECTOR_DIR/stage6d_projected_mha_h2_d16.mem" || failures=$((failures + 1))

{
  echo
  echo "Run markers:"
  for log in "$BUILD_DIR"/*_run.log; do
    grep -E "(STAGE6D_|CHECK_FAIL)" "$log" || true
  done
  echo "result=$([ "$failures" -eq 0 ] && echo PASS || echo FAIL)"
  echo "Full logs: build/stage6d_rtl_sim/*_compile.log and *_run.log"
} >> "$SUMMARY"
