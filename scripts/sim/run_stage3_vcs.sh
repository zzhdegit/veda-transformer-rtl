...
BUILD_DIR="$ROOT_DIR/build/stage3_rtl_sim"
REPORT_DIR="$ROOT_DIR/reports/stage_03"
...
{
  echo "Stage 3 VCS RTL simulation"
  echo "Build dir: build/stage3_rtl_sim"
...
compile_and_run() {
  ...
  timeout 360s "$simv" \
    +EXP_VECTOR_FILE="$VECTOR_DIR/stage3_exp_vectors.mem" \
    +RECIP_VECTOR_FILE="$VECTOR_DIR/stage3_recip_vectors.mem" \
    +ATTENTION_VECTOR_FILE="$VECTOR_DIR/stage3_attention_d8.mem" \
    -l "$run_log"
  local run_code=$?
  local log_errors=0
  if grep -E "(STAGE3_.*FAIL|CHECK_FAIL|Fatal:|assert.*failed|unsupported .* assertion failed)" "$run_log" >/dev/null 2>&1; then
    log_errors=1
  fi
...
compile_and_run_attention() {
  ...
  timeout 360s "$simv" \
    +ATTENTION_VECTOR_FILE="$VECTOR_DIR/$vector_file" \
    -l "$run_log"
  local run_code=$?
  local log_errors=0
  if grep -E "(STAGE3_.*FAIL|CHECK_FAIL|Fatal:|assert.*failed|unsupported .* assertion failed)" "$run_log" >/dev/null 2>&1; then
...
compile_and_run fp32_exp_recip tb_fp32_exp_recip_wrappers \
  "${DW_FILES[@]}" \
  "${COMMON_RTL[@]}" \
  "$ROOT_DIR/rtl/arithmetic/fp32_exp_wrapper.sv" \
  "$ROOT_DIR/rtl/arithmetic/fp32_recip_wrapper.sv" \
  "$ROOT_DIR/tb/rtl/stage3/tb_fp32_exp_recip_wrappers.sv" || failures=$((failures + 1))

compile_and_run_attention single_head_attention_d8 8 stage3_attention_d8.mem || failures=$((failures + 1))
compile_and_run_attention single_head_attention_d16 16 stage3_attention_d16.mem || failures=$((failures + 1))

{
  echo
  echo "Run markers:"
  for log in "$BUILD_DIR"/*_run.log; do
    grep -E "(STAGE3_|CHECK_FAIL)" "$log" || true
  done
  echo "result=$([ "$failures" -eq 0 ] && echo PASS || echo FAIL)"
  echo "Full logs: build/stage3_rtl_sim/*_compile.log and *_run.log"
} >> "$SUMMARY"
