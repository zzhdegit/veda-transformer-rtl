...
BUILD_DIR="$ROOT_DIR/build/stage7c_rtl_sim"
REPORT_DIR="$ROOT_DIR/reports/stage_07"
SUMMARY="$REPORT_DIR/phase_7c_vcs_rtl_sim.txt"
...
{
  echo "Stage 7C FFN/ReLU RTL simulation"
  echo "Build dir: build/stage7c_rtl_sim"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/sim/gen_stage7c_vectors.py" "$VECTOR_DIR" > "$BUILD_DIR/vector_gen.log" 2>&1
vector_code=$?
...
compile_and_run() {
  ...
  (cd "$BUILD_DIR" && vcs -full64 -sverilog -debug_access+pp -assert svaext -timescale=1ns/1ps \
    ...
    -o "$simv" \
    "${DW_FILES[@]}" "${RTL_FILES[@]}" \
    "$ROOT_DIR/tb/rtl/stage7/tb_stage7c_ffn_engine.sv" \
    -top tb_stage7c_ffn_engine \
    -l "$compile_log")
  local compile_code=$?
  ...
  timeout 600s "$simv" "+STAGE7C_VECTOR_FILE=$vector_file" -l "$run_log"
  local run_code=$?
  local log_errors=0
  if grep -E "(STAGE7C_.*FAIL|CHECK_FAIL|Fatal:|Error:|assert.*failed|unsupported .* assertion failed)" "$run_log" >/dev/null 2>&1; then
    log_errors=1
  fi
...
compile_and_run ffn_d8 8 "$VECTOR_DIR/stage7c_d8.mem" || failures=$((failures + 1))
compile_and_run ffn_d16 16 "$VECTOR_DIR/stage7c_d16.mem" || failures=$((failures + 1))

{
  echo
  echo "Run markers:"
  for log in "$BUILD_DIR"/*_run.log; do
    grep -E "(STAGE7C_|CHECK_FAIL)" "$log" || true
  done
  echo "result=$([ "$failures" -eq 0 ] && echo PASS || echo FAIL)"
  echo "Full logs: build/stage7c_rtl_sim/*_compile.log and *_run.log"
} >> "$SUMMARY"
