...
REPORT = ROOT / "reports/stage_06/phase_6a_test_results.txt"
...
def run_python_test_fallback(test_files):
    ...
    for index, path in enumerate(test_files):
        module_name = "_stage6a_fallback_%d_%s" % (index, path.stem)
...
def main():
    ...
    if shutil.which("pytest") or run([sys.executable, "-m", "pytest", "--version"])[0] == 0:
        ...
        lines.append("$ fallback-python-test-runner stage5/stage6a model tests")
        lines.append(output)
