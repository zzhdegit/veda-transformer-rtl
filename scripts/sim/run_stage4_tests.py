...
REPORT = ROOT / "reports/stage_04/test_results.txt"
...
def run_python_test_fallback(test_files):
    ...
    for index, path in enumerate(test_files):
        module_name = "_stage4_fallback_%d_%s" % (index, path.stem)
...
def main():
    ...
    if shutil.which("pytest") or run([sys.executable, "-m", "pytest", "--version"])[0] == 0:
        ...
        lines.append("$ fallback-python-test-runner stage1b/stage2/stage3/stage4 model tests")
        lines.append(output)
    ...
    if shutil.which("vcs"):
        cmd = ["bash", str(ROOT / "scripts" / "sim" / "run_stage4_vcs.sh")]
        code, output = run(cmd)
        lines.append("$ " + " ".join(cmd))
        lines.append(output.rstrip())
        lines.append("exit_code=%d" % code)
        ...
        lines.append("Stage 4 RTL simulation: SKIPPED - vcs executable not found on host.")
