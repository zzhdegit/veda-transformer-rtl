...
BUILD_DIR="$ROOT_DIR/build/stage5_rtl_sim"
REPORT_DIR="$ROOT_DIR/reports/stage_05"
...
{
  echo "Stage 5 VCS RTL simulation"
  echo "Build dir: build/stage5_rtl_sim"
...
compile_and_run_cache() {
  ...
  (cd "$BUILD_DIR" && vcs -full64 -sverilog -debug_access+pp -assert svaext -timescale=1ns/1ps \
    ...
    -o "$simv" \
    "$ROOT_DIR/rtl/cache/multi_head_kv_cache_manager.sv" \
    "$ROOT_DIR/tb/rtl/stage5/tb_multi_head_kv_cache_manager.sv" \
    -top tb_multi_head_kv_cache_manager \
    -l "$compile_log")
  local compile_code=$?
...
compile_and_run_cache multi_head_kv_cache_manager || failures=$((failures + 1))
compile_and_run_generation multi_head_generation_h1_d8 1 8 stage5_generation_h1_d8.mem || failures=$((failures + 1))
compile_and_run_generation multi_head_generation_h2_d8 2 8 stage5_generation_h2_d8.mem || failures=$((failures + 1))
compile_and_run_generation multi_head_generation_h4_d8 4 8 stage5_generation_h4_d8.mem || failures=$((failures + 1))
compile_and_run_generation multi_head_generation_h2_d16 2 16 stage5_generation_h2_d16.mem || failures=$((failures + 1))

{
  echo
  echo "Run markers:"
  for log in "$BUILD_DIR"/*_run.log; do
    grep -E "(STAGE5_|CHECK_FAIL)" "$log" || true
  done
  echo "result=$([ "$failures" -eq 0 ] && echo PASS || echo FAIL)"
  echo "Full logs: build/stage5_rtl_sim/*_compile.log and *_run.log"
} >> "$SUMMARY"
