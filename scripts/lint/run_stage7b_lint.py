...
REPORT = ROOT / "reports/stage_07/phase_7b_lint_results.txt"
...
def static_hygiene():
    ...
    required = [
        ...
        "D_MODEL must be power of two",
    ]
    for token in required:
        if token not in all_text:
            errors.append("missing Stage 7B RTL/assertion token: %s" % token)
    return errors
...
def main():
    ...
    lines.append("Static Stage 7B RTL hygiene: %s" % ("FAIL" if errors else "PASS"))
    lines.extend(errors)

    vlogan = shutil.which("vlogan")
    if vlogan:
        out_dir = ROOT / "build" / "stage7b_vlogan_lint"
        ...
        lines.append("$ vlogan -full64 -sverilog -work work <DW sim files> <stage7b RTL files>")
        lines.append("vlogan_log=build/stage7b_vlogan_lint/vlogan.log")
