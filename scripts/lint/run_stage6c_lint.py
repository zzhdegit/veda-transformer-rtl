...
REPORT = ROOT / "reports/stage_06/phase_6c_lint_results.txt"
...
RTL_FILES = [
    ...
    ROOT / "rtl/projection/qkv_projection_engine.sv",
]
...
def static_hygiene():
    ...
    required = [
        ...
        "no_projection_start_without_complete_input",
        "qkv_output_head_dim_order_legal",
        "no_attention_start_before_qkv_complete",
        "output stable until ready",
        "weight_matrix_complete_before_start",
    ]
    for token in required:
        if token not in all_text:
            errors.append("missing Stage 6C RTL/assertion token: %s" % token)
    qkv_text = (ROOT / "rtl/projection/qkv_projection_engine.sv").read_text(encoding="utf-8")
    if len(re.findall(r"\bprojection_controller\s*#", qkv_text)) != 1:
        errors.append("qkv_projection_engine must instantiate exactly one projection_controller")
    if len(re.findall(r"\bfp32_to_fp16\s*#", qkv_text)) != 1:
        errors.append("qkv_projection_engine must instantiate exactly one fp32_to_fp16 quantizer")
    return errors
...
def main():
    ...
    lines.append("Static Stage 6C RTL hygiene: %s" % ("FAIL" if errors else "PASS"))
    lines.extend(errors)

    vlogan = shutil.which("vlogan")
    if vlogan:
        out_dir = ROOT / "build" / "stage6c_vlogan_lint"
        ...
        lines.append("$ vlogan -full64 -sverilog -work work <DW sim files> <stage6c RTL files>")
        lines.append("vlogan_log=build/stage6c_vlogan_lint/vlogan.log")
