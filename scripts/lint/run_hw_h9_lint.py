#!/usr/bin/env python3
"""Hardware Stage H9 static and vlogan checks."""
...
REPORT = ROOT / "reports/hw_h9/lint_results.txt"
...
def static_hygiene():
    ...
    for token in required:
        if token not in all_text:
            errors.append("missing Hardware Stage H9 token: %s" % token)
    return errors
...
def main():
    ...
    lines = ["Hardware Stage H9 static RTL hygiene: %s" % ("FAIL" if errors else "PASS")]
    lines.extend(errors)

    vlogan = shutil.which("vlogan")
    if vlogan:
        out_dir = ROOT / "build/hw_h9_vlogan_lint"
        ...
        lines.append("$ vlogan -full64 -sverilog -work work <DW sim files> <hw-h9 RTL files>")
        lines.append("vlogan_log=build/hw_h9_vlogan_lint/vlogan.log")
