...
REPORT = ROOT / "reports/stage_07/phase_7c_lint_results.txt"
...
def static_hygiene():
    ...
    for token in required:
        if token not in all_text:
            errors.append("missing Stage 7C RTL/assertion token: %s" % token)
    if len(re.findall(r"\breconfigurable_pe_core\s*#", (ROOT / "rtl/transformer/ffn_engine.sv").read_text(encoding="utf-8"))) != 1:
...
def main():
    ...
    lines.append("Static Stage 7C RTL hygiene: %s" % ("FAIL" if errors else "PASS"))
    lines.extend(errors)

    vlogan = shutil.which("vlogan")
    if vlogan:
        out_dir = ROOT / "build" / "stage7c_vlogan_lint"
        ...
        lines.append("$ vlogan -full64 -sverilog -work work <DW sim files> <stage7c RTL files>")
        lines.append("vlogan_log=build/stage7c_vlogan_lint/vlogan.log")
