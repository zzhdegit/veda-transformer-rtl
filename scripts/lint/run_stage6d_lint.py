...
REPORT = ROOT / "reports/stage_06/phase_6d_lint_results.txt"
...
RTL_FILES = [
    ...
    ROOT / "rtl/attention/projected_multi_head_attention.sv",
]
...
def static_hygiene():
    ...
    top_text = (ROOT / "rtl/attention/projected_multi_head_attention.sv").read_text(encoding="utf-8")
    if len(re.findall(r"\bqkv_projection_engine\s*#", top_text)) != 1:
        errors.append("projected_multi_head_attention must instantiate exactly one qkv_projection_engine")
    if len(re.findall(r"\bmulti_head_generation_engine\s*#", top_text)) != 1:
        errors.append("projected_multi_head_attention must instantiate exactly one Stage 5 engine")
    for token in (
        "d_model_equals_n_head_times_d_head",
        "qkv stream stable under backpressure",
        "no_unknown_output_when_valid",
        "current_valid_seq_len",
    ):
        if token not in all_text:
            errors.append("missing Stage 6D RTL/assertion token: %s" % token)
    return errors
...
def main():
    ...
    lines.append("Static Stage 6D RTL hygiene: %s" % ("FAIL" if errors else "PASS"))
    lines.extend(errors)

    vlogan = shutil.which("vlogan")
    if vlogan:
        out_dir = ROOT / "build" / "stage6d_vlogan_lint"
        ...
        lines.append("$ vlogan -full64 -sverilog -work work <DW sim files> <stage6d RTL files>")
        lines.append("vlogan_log=build/stage6d_vlogan_lint/vlogan.log")
