...
REPORT = ROOT / "reports/stage_04/lint_results.txt"
...
RTL_FILES = [
    ...
    ROOT / "rtl/cache/kv_address_generator.sv",
    ROOT / "rtl/cache/kv_cache_manager.sv",
    ROOT / "rtl/cache/generation_attention_controller.sv",
    ROOT / "rtl/attention/generation_attention_engine.sv",
]
...
def static_hygiene():
    ...
    for token in (
        "no_read_beyond_valid_seq_len",
        "no_append_beyond_capacity",
        "no_attention_start_during_incomplete_append",
        ...
        "K/V token alignment",
        "no overwrite of valid token",
        "output stable under backpressure",
        "transaction count conserved",
        "no unknown output when valid",
        "cache address in range",
    ):
        if token not in all_text:
            errors.append("missing Stage 4 assertion/stability token: %s" % token)
    return errors
...
def main():
    ...
    lines.append("Static Stage 4 RTL hygiene: %s" % ("FAIL" if errors else "PASS"))
    lines.extend(errors)

    vlogan = shutil.which("vlogan")
    if vlogan:
        out_dir = ROOT / "build" / "stage4_vlogan_lint"
        ...
        lines.append("$ vlogan -full64 -sverilog -work work <DW sim files> <stage4 RTL files>")
        lines.append("vlogan_log=build/stage4_vlogan_lint/vlogan.log")
