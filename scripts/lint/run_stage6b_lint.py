...
REPORT = ROOT / "reports/stage_06/phase_6b_lint_results.txt"
...
def static_hygiene():
    ...
    for token in required:
        if token not in all_text:
            errors.append("missing Stage 6B RTL/assertion token: %s" % token)
    gemv_text = (ROOT / "rtl/projection/shared_gemv_projection_core.sv").read_text(encoding="utf-8")
    if len(re.findall(r"\breconfigurable_pe_core\s*#", gemv_text)) != 1:
        errors.append("Stage 6B shared GEMV must instantiate exactly one reconfigurable_pe_core")
    return errors
...
def main():
    ...
    lines.append("Static Stage 6B RTL hygiene: %s" % ("FAIL" if errors else "PASS"))
    lines.extend(errors)

    vlogan = shutil.which("vlogan")
    if vlogan:
        out_dir = ROOT / "build" / "stage6b_vlogan_lint"
        ...
        lines.append("$ vlogan -full64 -sverilog -work work <DW sim files> <stage6b RTL files>")
        lines.append("vlogan_log=build/stage6b_vlogan_lint/vlogan.log")
