#!/usr/bin/env python3
"""Stage 8D static and vlogan checks for paper-array attention integration."""
...
REPORT = ROOT / "reports/stage_08/phase_8d_lint_results.txt"
...
def static_hygiene():
    ...
    controller = (ROOT / "rtl/attention/single_head_attention_controller.sv").read_text(encoding="utf-8")
    adapter = (ROOT / "rtl/attention/paper/paper_attention_adapter.sv").read_text(encoding="utf-8")
    top = (ROOT / "rtl/transformer/transformer_layer.sv").read_text(encoding="utf-8")
    required = [
        "ATTENTION_PE_ARCH",
        "LEGACY_PE",
        "PAPER_ARRAY",
        ...
        "output payload stable",
    ]
    for token in required:
        if token not in all_text:
            errors.append("missing Stage 8D integration token: %s" % token)
    if len(re.findall(r"\breconfigurable_pe_core\s*#", controller)) != 1:
        errors.append("single_head_attention_controller must keep exactly one legacy PE branch")
    if len(re.findall(r"\bpaper_attention_adapter\s*#", controller)) != 1:
        errors.append("single_head_attention_controller must keep exactly one paper adapter branch")
    if len(re.findall(r"\bpaper_array_8x8x2\s*#", adapter)) != 1:
        errors.append("paper_attention_adapter must instantiate exactly one paper_array_8x8x2")
    if ".ATTENTION_PE_ARCH(ATTENTION_PE_ARCH)" not in top:
        errors.append("transformer_layer must pass ATTENTION_PE_ARCH to projection_integrated_mha")
    return errors
...
def main():
    ...
    lines = ["Static Stage 8D RTL hygiene: %s" % ("FAIL" if errors else "PASS")]
    lines.extend(errors)

    vlogan = shutil.which("vlogan")
    if vlogan:
        out_dir = ROOT / "build/stage8d_vlogan_lint"
        ...
        lines.append("$ vlogan -full64 -sverilog -work work <DW sim files> <stage8d RTL files>")
        lines.append("vlogan_log=build/stage8d_vlogan_lint/vlogan.log")
