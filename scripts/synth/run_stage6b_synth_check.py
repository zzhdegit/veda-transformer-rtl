...
REPORT = ROOT / "reports/stage_06/phase_6b_synth_check.txt"
...
def main():
    ...
    lines = [
        "Stage 6B synthesis/elaboration check",
    ...
    if not dc_shell:
        ...
        work_dir = ROOT / "build" / "stage6b_dc_run"
        work_dir.mkdir(parents=True, exist_ok=True)
        env = os.environ.copy()
        env["DW_FOUNDATION_SLDB"] = dw_sldb
        cmd = [dc_shell, "-f", str(ROOT / "scripts" / "synth" / "stage6b_elaborate.tcl")]
        proc = subprocess.run(
            cmd,
            cwd=str(work_dir),
            env=env,
        ...
        dc_log = ROOT / "reports" / "stage_06" / "phase_6b_dc_elaborate.log"
        dc_log.write_text(proc.stdout, encoding="utf-8", errors="ignore")
        dc_errors = [
            line for line in proc.stdout.splitlines()
            if re.search(r"^(Error:|\*\*\* .*errors?)", line)
        ]
        lines.append("$ dc_shell -f scripts/synth/stage6b_elaborate.tcl")
        ...
        lines.append("DC log: reports/stage_06/phase_6b_dc_elaborate.log")
        ...
        lines.append("Checked fp32_to_fp16, shared_gemv_projection_core, and projection_controller.")
        lines.append("Behavioral projection memories remain provisional; no SRAM macro or PPA conclusion is produced.")
