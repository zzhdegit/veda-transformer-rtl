...
REPORT = ROOT / "reports/stage_06/phase_6e_synth_check.txt"
...
def main():
    ...
    lines = [
        "Stage 6E synthesis/elaboration check",
    ...
    if not dc_shell:
        ...
        work_dir = ROOT / "build" / "stage6e_dc_run"
        work_dir.mkdir(parents=True, exist_ok=True)
        env = os.environ.copy()
        env["DW_FOUNDATION_SLDB"] = dw_sldb
        cmd = [dc_shell, "-f", str(ROOT / "scripts" / "synth" / "stage6e_elaborate.tcl")]
        proc = subprocess.run(
            cmd,
            cwd=str(work_dir),
            env=env,
        ...
        dc_log = ROOT / "reports" / "stage_06" / "phase_6e_dc_elaborate.log"
        dc_log.write_text(proc.stdout, encoding="utf-8", errors="ignore")
        dc_errors = [
            line for line in proc.stdout.splitlines()
            if re.search(r"^(Error:|\*\*\* .*errors?)", line)
        ]
        lines.append("$ dc_shell -f scripts/synth/stage6e_elaborate.tcl")
        ...
        lines.append("DC log: reports/stage_06/phase_6e_dc_elaborate.log")
        ...
        lines.append("Checked projection_integrated_mha H1/D8, H2/D8, H4/D8, and H2/D16 full-top configs.")
        lines.append("Checked D_MODEL=128 address/control elaboration on projection_input_buffer, shared_gemv_projection_core, qkv_staging_buffer, head_concat_quantizer, concat_fp16_buffer, and output_projection_controller.")
        lines.append("The D_MODEL=128 check intentionally does not elaborate full behavioral projection weight storage as physical memory.")
        lines.append("Behavioral projection, concat, and cache memories remain provisional; no SRAM macro or PPA conclusion is produced.")
