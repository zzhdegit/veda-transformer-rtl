...
REPORT = ROOT / "reports/stage_07/phase_7b_synth_check.txt"
...
def main():
    ...
    lines = [
        "Stage 7B synthesis/elaboration check",
    ...
    if not dc_shell:
        ...
        work_dir = ROOT / "build" / "stage7b_dc_run"
        work_dir.mkdir(parents=True, exist_ok=True)
        env = os.environ.copy()
        env["DW_FOUNDATION_SLDB"] = dw_sldb
        cmd = [dc_shell, "-f", str(ROOT / "scripts" / "synth" / "stage7b_elaborate.tcl")]
        proc = subprocess.run(
            cmd,
            cwd=str(work_dir),
            env=env,
        ...
        dc_log = ROOT / "reports" / "stage_07" / "phase_7b_dc_elaborate.log"
        dc_log.write_text(proc.stdout, encoding="utf-8", errors="ignore")
        dc_errors = [
            line for line in proc.stdout.splitlines()
            if re.search(r"^(Error:|\*\*\* .*errors?)", line)
        ]
        lines.append("$ dc_shell -f scripts/synth/stage7b_elaborate.tcl")
        ...
        lines.append("DC log: reports/stage_07/phase_7b_dc_elaborate.log")
        ...
        lines.append("Checked fp32_sqrt_wrapper, rmsnorm_engine, and residual_add_engine.")
