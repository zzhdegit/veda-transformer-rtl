...
REPORT = ROOT / "reports/stage_02/synth_check.txt"
...
def main():
    ...
    lines = [
        "Stage 2 synthesis/elaboration check",
    ...
    if not dc_shell:
        ...
        work_dir = ROOT / "build" / "stage2_dc_run"
        work_dir.mkdir(parents=True, exist_ok=True)
        env = os.environ.copy()
        env["DW_FOUNDATION_SLDB"] = dw_sldb
        cmd = [dc_shell, "-f", str(ROOT / "scripts" / "synth" / "stage2_elaborate.tcl")]
        proc = subprocess.run(
            cmd,
            cwd=str(work_dir),
            env=env,
        ...
        dc_log = ROOT / "reports" / "stage_02" / "dc_elaborate.log"
        dc_log.write_text(proc.stdout, encoding="utf-8", errors="ignore")
        lines.append("$ dc_shell -f scripts/synth/stage2_elaborate.tcl")
        ...
        lines.append("DC log: reports/stage_02/dc_elaborate.log")
        ...
        lines.append("Checked default PE_NUM=8 top and parameterized PE_NUM=128 core.")
